...
class CifRepeat {
    question(dataInitial, monthSelected, readPath, writePath) {
        ...
        dataInitial
            ...
            .forEach((item) => {
                item.arrayTenderResult.forEach((tender) => {
                    const resultOld = oldDataJson.find(
                        (oldItem) => oldItem.PartyIdentification === tender.PartyIdentification
                    );
                    if (resultOld) {
